...
cfg_if! {
    if #[cfg(feature = "whisper_cpp_vulkan")] {
        /// Errors that can be generated by the [Whisper::transcribe] method
        #[derive(Error, Debug)]
        pub enum Error {
        ...
        pub enum Error {
            ...
            AudioDuration,
            #[error(transparent)]
            /// Missing paramters to instantiate the burn backend
            ComputeBuilder(#[from] TranscribeBuilderError),
            #[error("Failed to initialize the tokenizer. Reason {0}")]
            /// Huggingface tokenizer encountered error class
            Tokenizer(String),
            /// The model config file error class
            #[error(transparent)]
            BurnConfig(#[from] burn::config::ConfigError),
            /// The model weigths file error class
            #[error(transparent)]
